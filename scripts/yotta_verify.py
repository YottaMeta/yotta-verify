...
MAX_FILES = 2000
# v0.3.3：不可静态分析的可执行 / 二进制载荷不再静默跳过（fail-closed）。
...
def find_opaque_files(root):
    """列出无法静态分析、但带可执行 / 二进制特征的文件（v0.3.3，fail-closed）。"""
    out = []
    if root.is_file():
...
def _safe_extract(tf, dest):
    """提取 tarball（Python 3.8 兼容；拒绝路径穿越、链接与特殊文件；v0.3.3 加解包上限）。"""
    safe_members = []
...
def scan_core(target, name_hint=None, auto_name_hint=False):
    ...
    # v0.3.3：不可静态分析的可执行 / 二进制载荷按 fail-closed 处理（不再静默跳过）
